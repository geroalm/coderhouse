...
export class ProductsManagerMongo{
    ...
    async getProducts(onlyActive){ //agregar filtro por status
        try {
            let productsList= null;
            if(onlyActive) productsList = await this.model.find({status:true});
            else productsList = await this.model.find();
            return productsList;
        } catch (error) {
            console.log(error)
